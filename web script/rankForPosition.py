...
s = "SELECT `accession_id`,`position_asc_id`,`position_desc_id`,`gene_symbol_id`,`site_id`,`histology_id` from all_info_order_by_position;"
cursor.execute(s)
d = cursor.fetchall()
accession_id = []
position_asc_id = []
...
me = []
for x in methods:
	sql = "SELECT accession_id,CASE WHEN @prevRank = `%s` THEN @curRank WHEN @prevRank := `%s` THEN @curRank := @curRank + 1 END AS rank FROM all_info_order_by_position, (SELECT @curRank :=0, @prevRank := NULL) r ORDER BY `%s`;"%(x,x,x)
	cursor.execute(sql)
...
result.to_csv('rankForPosition.tsv',sep='\t',header=False,index=False)
